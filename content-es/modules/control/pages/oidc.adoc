...
Puede obtener más información sobre la configuración aquí: https://docs.jmix.ru/jmix/oidc/index.html#client-configuration[documentación oficial].
...
Para explorar el complemento Jmix OIDC, puede ejecutar Keycloak localmente utilizando Docker.
...
Consulte la https://www.keycloak.org/getting-started/getting-started-docker[documentación de Keycloak] para obtener más detalles.
...
Puede leer sobre la configuración de una instancia de Keycloak en la https://www.keycloak.org/docs/latest/server_admin/[Guía de administración del servidor].
...
+
Después de guardar el cliente, vaya a la pestaña *Credentials* y copie el *Client Secret*.
...
1. Vaya a *Client scopes* → `roles` → *Mappers* → `realm roles`.
