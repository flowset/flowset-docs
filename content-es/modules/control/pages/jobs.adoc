...
La sección *Jobs* permite ver y gestionar los trabajos asíncronos del motor BPM.
...
* en la pantalla dedicada <<jobs-list-view,Jobs>> — para todos los trabajos del motor;
...
image::jobs-screan.png[align="center", width="1552"]
...
. Selecciona un trabajo con un valor de *Retries* de `0` y haz clic en *Retry*.
. En el cuadro de diálogo que se abre, especifica el número de intentos de ejecución en el campo *Retries* (el valor predeterminado es `1`; el valor debe ser positivo).
+
image::job-retry-dialog.png[align="center", width="720"]
. Haz clic en *Retry* para aplicar el nuevo número de intentos.

[[job-details]]
== Vista detallada del trabajo

Para abrir información detallada sobre un trabajo, haz clic en su identificador o utiliza la acción *View*.
...
La acción *Set priority* te permite establecer una nueva prioridad de ejecución para el trabajo en el campo *New priority*. La prioridad afecta al orden en que el Job Executor toma los trabajos.
...
Para un trabajo de *temporizador*, puedes elegir la operación (*Operation*):
...
* *Recalculate* — recalcular la fecha de vencimiento utilizando el motor. Cuando se selecciona esta operación, puedes especificar la fecha base para el recálculo — la *current time* o la *creation time* del trabajo.
