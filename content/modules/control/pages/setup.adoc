...
. Unpack the archive into any working directory. Keep the folder structure intact — the `start`/`stop` scripts must stay next to `server.tar.gz`. On Windows the built-in Explorer extractor works fine here, because the archive contains only short paths.
